...
class DataFeeder(threading.Thread):
  ...
  def __init__(self, coordinator, metadata_filename, hparams):
    ...
    self._placeholders = [
      tf.placeholder(tf.float32, [None, None, hparams.num_mels], 'inputs'),
    ...
    #  print('hparams.use_cmudict',hparams.use_cmudict)
    #  print('self._datadir',self._datadir)
  ...
  def _get_next_example(self):
    ...
    input_data = np.load(os.path.join(self._datadir, meta[0]))
    mel_target = np.load(os.path.join(self._datadir, meta[3]))
    linear_target = np.load(os.path.join(self._datadir, meta[2]))
...
def _prepare_batch(batch, outputs_per_step):
  ...
  input_lengths = np.asarray([x[0].shape[0] for x in batch], dtype=np.int32)
